...
NKTrajectory::State NKTrajectory::State::Interpolate(const NKTrajectory::State& other,
                                                     second_t                   newT) const
{
    double scale = ((newT - t) / (other.t - t)).value();
    return NKTrajectory::State{newT, pose.Exp(pose.Log(other.pose) * scale),
                               (other.vx - vx) * scale + vx, (other.vy - vy) * scale + vy,
...
NKTrajectory::State NKTrajectory::Sample(second_t t) const
{
    ...
    auto upper = std::upper_bound(m_states.begin(), m_states.end(), t,
...
Pose2d NKTrajectory::GetInitialPose() const
{
    return m_states.at(0).pose;
}

second_t NKTrajectory::GetTotalTime() const
{
...
void from_json(const json& j, NKTrajectory::State& state)
{
    ...
    state.pose =
        Pose2d(Translation2d(meter_t{j.at("x").get<double>()}, meter_t{j.at("y").get<double>()}),
...
void to_json(json& j, const NKTrajectory& traj)
{

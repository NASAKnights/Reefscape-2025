    ...
    , m_steerSimPosition(m_steerSim.GetDouble("Position"))
{
    ...
    steerConfig.Feedback.FeedbackSensorSource   = signals::FeedbackSensorSourceValue::FusedCANcoder;
...
{
    frc::SmartDashboard::PutNumber("Module " + std::to_string(m_id) + "/" + " Reported Angle",
                                   GetRotation().Degrees().value());
    frc::SmartDashboard::PutNumber("Module " + std::to_string(m_id) + "/" + " CANCoder Angle",
                                   GetAbsoluteRotation().Degrees().value());
    frc::SmartDashboard::PutNumber("Module " + std::to_string(m_id) + "/" + " Velocity",
                                   (m_driveMotor.GetVelocity()).GetValue().value());
    frc::SmartDashboard::PutNumber("Module " + std::to_string(m_id) + "/" + " Rotations",
...
{
    return {m_driveMotor.GetVelocity().GetValue() * kDriveConversion, GetRotation()};
}

frc::SwerveModulePosition SwerveModule::GetPosition()
{
    return {m_driveMotor.GetPosition().GetValue() * kDriveConversion, GetRotation()};
}
